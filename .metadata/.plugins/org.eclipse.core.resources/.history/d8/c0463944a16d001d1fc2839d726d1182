...
// out array must be twice the length of in array
...
int LCD_setAddrWindow(SPI_HandleTypeDef* spi, uint16_t x1, uint16_t y1, uint16_t w, uint16_t h)
{
	...
	uint8_t bytes[4][2];
	uint16_to_bytes(x1, &bytes[0][0]);
	uint16_to_bytes(x2, &bytes[1][0]);
	uint16_to_bytes(y1, &bytes[2][0]);
	uint16_to_bytes(y2, &bytes[3][0]);

	LCD_startWrite();
	result = LCD_writeCommand(spi, HX8357_CASET); // Column address set
	if (result != HAL_OK) return 0;

	result = HAL_SPI_Transmit(spi, &bytes[0][0], 2, 0);
	if (result != HAL_OK) return 0;

	result = HAL_SPI_Transmit(spi, &bytes[0][1], 2, 0);
	if (result != HAL_OK) return 0;

	result = LCD_writeCommand(spi, HX8357_PASET); // Row address set
	if (result != HAL_OK) return 0;

	result = HAL_SPI_Transmit(spi, &bytes[0][2], 2, 0);
	if (result != HAL_OK) return 0;

	result = HAL_SPI_Transmit(spi, &bytes[0][3], 2, 0);
	if (result != HAL_OK) return 0;
...
int LCD_pushColor(SPI_HandleTypeDef* spi, uint16_t color) {
	...
	LCD_startWrite();
	result = HAL_SPI_Transmit(spi, &bytes[0], 2, 0);
	LCD_endWrite();
	return result == HAL_OK;
}
...
int LCD_pushColors(SPI_HandleTypeDef* spi, uint16_t* colors, uint32_t len) {
	HAL_StatusTypeDef result;
	uint8_t *bytes = (uint8_t*)malloc(2*len);
	LCD_startWrite();
	uint16a_to_bytes(&colors[0], &bytes[0], len);
	result = HAL_SPI_Transmit(spi, &bytes[0], 2*len, 0);
	LCD_endWrite();
	free(bytes);
	return result == HAL_OK;
}
...
int LCD_writePixels(SPI_HandleTypeDef* spi, uint16_t* colors,
					int16_t x, int16_t y, int16_t w, int16_t h)
{
	if ((x < 0) ||(x >= HX8357_TFTWIDTH) || (y < 0) || (y >= HX8357_TFTHEIGHT)) return 0;
	if(!LCD_setAddrWindow(spi, x, y, w, h)) return 0;
	if (!LCD_pushColors(spi, colors, w*h)) return 0;
	return 1;
}
